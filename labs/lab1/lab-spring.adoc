...
----
+
. open _manifest.yml_ in a text editor.  Look for the following line:
+
----
- name: mvc-demo-yournamehere
----
. replace `yournamehere` with your name...there.  Save and exit.
...
Here the droplet is being uploaded to Pivotal Cloudfoundry's internal blobstore so that it can be easily copied to one or more _https://docs.cloudfoundry.org/concepts/diego/diego-architecture.html#cell-components[Diego Cells]_ for execution.
